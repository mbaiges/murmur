...
| **Vulgar** | Very rude informal register (UI label **Vulgar**); prompt uses **turro**-style wording; with **Argentinian Spanish**, rioplatense turro slang (see `toneInstructions.ts`). |
...
5. **Tone & language** offers **No tone**, **Neutral**, **Professional**, **Vulgar**, and **Custom** with text field when Custom; includes **Argentinian Spanish** among output languages. Tone affects generation without appearing in the prompt textarea. **Custom** with empty text **cannot be saved**.
