...
import { migrateRawConfigToLatest } from '../../../../../src/core/lib/config/configMigrate'
import { MurmurConfigSchema } from '../../../../../src/core/domain/config.schema'

describe('configMigrate', () => {
  it('migrates v1 flat config to v3 with profile on monitors', () => {
    const raw = {
      geminiApiKey: 'key',
    ...
    const migrated = migrateRawConfigToLatest(raw)
    ...
    expect(parsed.monitors[0].profile.backgroundMode).toBe('gradient')
